...
# === find DHI/DNI with DIRINT model ===
...
# Compute DHI from : GHI = DHI + DNI * cos(zenith)
df['DHI'] = df['GHI'] - df['DNI'] * np.cos(np.radians(df['zenith']))
df['DHI'] = df['DHI'].clip(lower=0)  # avoid negative values
...
# Export
df.round(2).to_csv('/Volumes/evelyn_hv62/part3/data/RES/i/i_isotropic/ssp585/i_max.csv')
